...
class PrivateController extends AbstractController
{
    ...
    public function settings(): void
    {
        if (isset($_SESSION['email'])) {
            ...
            $this->render("settings.html.twig", []);
        } else {
            $this->render("home.html.twig", []);
        }
    }
    public function updateSettings(): void
    {
        ...
        if (isset($_SESSION['email'])) {
            ...
            if (isset($_POST['old-password']) || isset($_POST['avatar'])) {

                // check CSRF token
                $tokenManager = new CSRFTokenManager();
                if (isset($_POST["csrf-token"]) && $tokenManager->validateCSRFToken($_POST["csrf-token"])) {

                    //update avatar if it has been submitted
                    if (isset($_POST['avatar'])) {
                        $avatar = htmlspecialchars($_POST['avatar']);
                        //apply modifications in db
                        $um->updateUser($id, $_SESSION['email'], $_SESSION['name'], $avatar);
                        //apply modification to session
                        $_SESSION['avatar'] = $avatar;
                    }

                    //update password if a new one has been submitted
                    if (isset($_POST['old-password']) && isset($_POST['password']) && isset($_POST['password-confirm'])) {

                        $oldPassword = htmlspecialchars($_POST['old-password']);
                        $password = htmlspecialchars($_POST['password']);

                        //check old-password
                        $userFound = $um->findById($id);
                        $hashFound = $userFound->getPassword();
                        $isPasswordCorrect = password_verify($oldPassword, $hashFound);
                        if ($isPasswordCorrect) {
                            ...
                            $this->redirect("index.php?route=error&error=Wrong password");
                        }
                    } else {
                        $this->redirect("index.php?route=settings");
                    }
